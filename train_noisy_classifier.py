...
for iter_idx in trange(1, 1 + TOTAL_ITERS):
    ...
    if iter_idx % EVAL_FREQ == 0:
        with torch.no_grad():
            ...
            # hard_noise_t = (torch.ones(1080) * config.sde.N) - config.sde.N / 4 
            # noises = {'small': small_noise_t,
            #           'little': little_noise_t,
            #           'middle': middle_noise_t,
            #           'hard': hard_noise_t}
            noise_levels = {'small': 1,
                      'little': config.sde.N / 5,
                      'middle': config.sde.N / 2,
                      'hard': config.sde.N - config.sde.N / 4}
            
            # for name, noise_t in tqdm(noises.items()):
            for name, noise_level_t in tqdm(noise_levels.items()):
                valid_loss = 0
                valid_accuracy = 0
                ...
                for i, (X, y) in T:
                    ...
                    # create noise
                    noise = torch.randn_like(X)
                    noise_t = torch.ones(X.size()) * noise_level_t
